...
## v0.2.0-alpha.1 (2026-05-13)

### 新工具 (2 个)
- `k3cloud_create_from_template(templateId, newFormId, name, subSystemId)` — 通过 BOS 自带模板继承创建新 metaobject(账表 / 单据 / 基础资料)。复用 SaveForIDEV9 wire 路径,跟 k3cloud_create_extension 同一条路径。
...
### 新 skill
- `k3cloud/metaobject-creation-index` — 全 BOS_* 模板字典(账表 7 + 单据 5 + 基础资料 7) + 新建 vs 扩展决策树 + wire 格式参考。
...
- **7.4 账表 knowledge bridge** — events-reference 解锁(本 7.6 版完整闭环)
- **7.6 创建新 metaobject** — 通过 BOS 模板继承,2 工具 + 1 skill + 5 层测试全过 [本版亮点]

### 测试
73 test files / 883 tests, 全绿。lint clean。

### 已知限制
- 树形账表 / 直接SQL 账表 / 透视表 — 留 v0.3
- DynamicForm 创建 — 留 v0.3
