...
**最近一次更新**:2026-05-13(Plan 7.6 — 新建 metaobject 模板继承路径落地)
**对应仓库版本**:`main` @ HEAD(Plan 7.6:k3cloud_create_from_template + k3cloud_register_sysreport_python_plugins)
...
│
├─ X 是「新建 metaobject」(全新单据 / 基础资料 / 账表,非扩展旧对象)?
│  └─ ✅ Route B — SaveForIDEV9 + BaseObjectId=BOS_*Template(模板继承)
│                   跟扩展业务对象同 wire 路径,区别仅在 BaseObjectId 取值
│                   工具: k3cloud_create_from_template(templateId, newFormId, name, subSystemId)
│                   已覆盖模板: BOS_BillTemplate(100) / BOS_BaseDataTemplate(400) / BOS_SysReportTemplate(900)
│                   SysReport 附加插件工具: k3cloud_register_sysreport_python_plugins
│
...
**例外（Plan 7.6 新建 metaobject 路径）**:新建 metaobject 走 BOS_*Template 继承时,Step 2 capture 可用反编译替代 — 直接读 BOS server 拿全 19 个 BOS_* 模板字典(实证:Plan 7.6 反编译 `Kingdee.BOS.Core.Metadata.MetaObject.MetaObjectType`)。Step 4 wire-replay fixture 仍必须留(Plan 7.6 已落 5 个 fixture: 4 create + 1 register-plugin)。
