...
## §0 先分清注册路径(决定后面所有排查姿势)

本流程主体的示例是 **OpenDeploy 工具注册**的类形式(`class X(AbstractBillPlugIn)` + `self.View`)。

如果用户是在 **BOS 设计器 / 协同开发平台右键"注册Python脚本"**,那是**函数形式**:
`def AfterBindData(e):` + `this.View`,**不写 class、不用 self**。
类形式在这条路径下保存不报错但**永不执行**,连 `raise Exception` 都没反应 —— 此时不要在本流程的
§2/§3/§4 里打转,直接转 `references/known-pitfalls` §0-§2。

判断依据:问用户"你是用工具注册的,还是在 BOS 里点'注册Python脚本'?"。
...
改了 py 代码但行为没变 → 大概率缓存。按顺序做,前面的没做后面的白搭:

0. **协同开发平台:签入了吗?**(9.x 必查)只点"保存"只写本地工作区,**必须签入**才提交到服务器。
   插件配置行前的 `*` 就是"本地已改、未签入"的信号,`*` 消失才算数。
1. **关掉已打开的单据页签**:客户端按**页签粒度**缓存 Python 脚本。重开整个客户端或刷新页面
   都未必重新编译脚本,只有把页签点 X 关掉、从菜单重新进入才会重新加载。
2. **BOS Designer 缓存**:在 Designer 里打开扩展对象列表,**F5 刷新**
3. **客户端表单缓存**:用户已经打开的单据要**重登客户端**,F5 可能不够
4. **清缓存目录**:`%APPDATA%\Kingdee` 与客户端安装目录下的 `Cache`
5. **服务端插件缓存**:某些情况需要重启 IIS 应用池(生产环境慎用,告知用户自行决定)
